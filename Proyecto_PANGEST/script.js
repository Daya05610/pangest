...
document.addEventListener("DOMContentLoaded", () => {
    ...
    // --- C. CATÁLOGO INICIAL ---
    const guardados = localStorage.getItem("productosMenu");
    if (guardados) {
        productosGlobales = JSON.parse(guardados);
    } else {
        productosGlobales = productosDefecto;
        localStorage.setItem("productosMenu", JSON.stringify(productosGlobales));
    }
...
function confirmarCompra() {
    ...
    carrito.forEach(producto => {
        ...
        resumenProductos.push(`${producto.nombre} (x${producto.cantidad})`);
    });

    let ventasActuales = parseFloat(localStorage.getItem("ventasGlobales")) || 0;
    localStorage.setItem("ventasGlobales", ventasActuales + totalCompra);

    const ahora = new Date();
    const horaFormateada = ahora.toLocaleDateString('es-CO') + " — " + ahora.toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' });
